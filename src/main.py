...
def search_scripts(query):
    ...
    for root, dirs, files in os.walk("src/scripts"):
        for name in files:
            if name.endswith(".py"):
                scripts.append(os.path.join(root, name))
                print(f"{count}\t{os.path.join(root, name).replace(query, Fore.BLUE + query + Fore.RESET)}")
                count += 1
    return scripts
...
def terminal():
    ...
    while True:
        if current_script == "":
            ...
            command = input("py-scan(" + Fore.BLUE + current_script.replace("src/scripts/","") + Fore.RESET + ")> ")
